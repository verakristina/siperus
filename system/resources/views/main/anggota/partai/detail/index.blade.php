						...
						<div class="nav-tabs-custom tab-custom" style="box-shadow: 0 0 0 rgba(0,0,0,0);">
							...
							<div class="tab-content">
								<div class="active tab-pane" id="biodata">
									@include('main.user.detail.biodata')
								</div>
								<div class="tab-pane" id="riwayat">
									@include('main.user.detail.riwayat')
								</div>
								<div class="tab-pane" id="dokumen">
									@include('main.user.detail.dokumen')
								</div>
							</div>
						</div>
					</form>
...
$(document).ready(function(){ 
	...
	$('#tlProv').change(function(){
		var prov = $(this).val();
		changeKabupatenOptionKPU('#tlKab',prov);
	});
	$('#abProv').change(function(){
		var prov = $(this).val();
		changeKabupatenOptionKPU('#abKab',prov);
	});
	$('#abKab').change(function(){
		var prov = $('#abProv').val();
		var kab = $(this).val();
		changeKecamatanOptionKPU('#abKec',prov,kab);
	});
	$('#abKec').change(function(){
		var prov = $('#abProv').val();
		var kab = $('#abKab').val();
		var kec = $(this).val();
		changeKelurahanOptionKPU('#abKel',prov,kab,kec);
	});
